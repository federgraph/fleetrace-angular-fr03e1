...
export class TCalcEventProxy01 extends TCalcEventProxy {
  private TimeNull: number = Number.MAX_SAFE_INTEGER; // previously in TimeConst.TimeNull
  // in
  ...
  protected Calc_BTime(): void {
    if (this.BestOTime === this.TimeNull) {
      for (let i = 0; i < this.Count; i++) {
        this.BTime[i] = this.TimeNull;
      }
    } else {
